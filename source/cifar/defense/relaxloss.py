...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "-name",
        type=str,
        help="experiment name, used for set up save_dir",
    )
    parser.add_argument(
        "--dataset", type=str, choices=["CIFAR10", "CIFAR100"], help="dataset name"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--partition", type=str, choices=["target", "shadow"], help="training partition"
    )
    parser.add_argument("--if_resume", type=str2bool, help="If resume from checkpoint")
    parser.add_argument(
        "--if_data_augmentation", "-aug", type=str2bool, help="If use data augmentation"
    )
    parser.add_argument(
        "--if_onlyeval", type=str2bool, help="If only evaluate the pre-trained model"
    )
    return parser
...
def check_args(parser):
    ...
    if args.if_onlyeval:
        ...
        specific_configs = load_yaml(
            FILE_DIR + "/configs/%s_%s.yml" % (args.dataset, args.model)
        )
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            if epoch % 2 == 0:  # gradient ascent/ normal gradient descent
                ...
                if loss_ce > self.alpha:  # normal gradient descent
                    ...
                    confidence_target = self.softmax(outputs)[
                        torch.arange(targets.size(0)), targets
                    ]
                    confidence_target = torch.clamp(
                        confidence_target, min=0.0, max=self.upper
                    )
                    confidence_else = (1.0 - confidence_target) / (self.num_classes - 1)
                    onehot = one_hot_embedding(targets, num_classes=self.num_classes)
                    soft_targets = onehot * confidence_target.unsqueeze(-1).repeat(
                        1, self.num_classes
                    ) + (1 - onehot) * confidence_else.unsqueeze(-1).repeat(
                        1, self.num_classes
                    )
                    loss = (1 - correct) * self.crossentropy_soft(
                        outputs, soft_targets
                    ) - 1.0 * loss_ce_full
...
def main():
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for param_group in optimizer.param_groups:
            lr = param_group["lr"]
        logger.append(
            [lr, train_ce, test_ce, train_acc, test_acc, train_acc5, test_acc5]
        )
        print(
            "Epoch %d, Train acc: %f, Test acc: %f, lr: %f"
            % (epoch, train_acc, test_acc, lr)
        )
    ...
    plot_hist(
        ...
        ["train", "val"],
        os.path.join(save_dir, "hist_ep%d.png" % epoch),
    )
